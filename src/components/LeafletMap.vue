  ...
  <div class="map_parent">
    <div class="control_top">
      ...
      >
        <div
          class="layer_btn"
          :class="{ active: activeLayer === 'Mapbox' }"
          @click="setLayer('Mapbox')"
        >
          Mapbox
        </div>
        <div
          class="layer_btn"
          :class="{ active: activeLayer === 'Mapnik' }"
          @click="setLayer('Mapnik')"
        >
          Mapnik
        </div>
        <div
          class="layer_btn"
          :class="{ active: activeLayer === 'Mapbox sat' }"
          @click="setLayer('Mapbox sat')"
        >
          Mapbox sat
        </div>
        <div
          class="layer_btn layer_btn_last"
          :class="{ active: activeLayer === 'ESRI sat' }"
          @click="setLayer('ESRI sat')"
        >
          ESRI sat
        </div>
      </div>
      <router-link
        class="orm_control orm_info"
        to="/about"
        :title="$t('menu.about')"
      ></router-link>
      <div
        class="orm_control orm_layers popup_activator"
        :title="$t('menu.map_view')"
        :class="{ control_active: layersPopup }"
        @click="layersPopup = !layersPopup"
      ></div>
      <router-link
        class="orm_control orm_map_add_white add_green"
        to="/map/add"
        :title="$t('menu.add')"
      ></router-link>
    </div>
    ...
    <div class="control_bottom">
      <div
        class="orm_control orm_position"
        @click="showPosition"
        :title="$t('menu.my_location')"
      ></div>
...
export default {
  ...
  mounted() {
    const defaultCoords = this.getDefaultCoords(this.$i18n.locale);
    const lat =
    ...
    const defLayer = localStorage.getItem("layer") || "Mapbox";
