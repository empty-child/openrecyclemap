...
import FractionsForm from "./FractionsForm.vue";
...
export default {
  ...
  methods: {
    ...
    displayData: function (data, filter, to_select) {
      ...
      const ovData = osmtogeojson(data);
    ...
    loadData: function (params) {
      this.loading = true;
      const to_select =
        params && params.node ? params.type + "/" + params.node : false;
      this.fetchAmenity(
        this.map.getCenter(),
    ...
    saveData: function (event) {
      const nposition = this.marker ? this.marker.getLatLng() : null;
      this.disableAddMode();
      ...
      if (event.id) {
        const position = this.selectedLayer
    ...
    pushPosition: function () {
      ...
      const lon = position.lng.toFixed(5);
      if (
    ...
    onMapChange: function () {
      ...
      const zoomInvalid = this.map.getZoom() < 13;
      const fit = this.bounds.contains(this.map.getCenter());
